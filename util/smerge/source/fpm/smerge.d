...
public string[string] generateAnchorMap(string baseDir, Config config = Config()) {
	...
	string currentValue() {
		...
		foreach (string namedPath; config.nameDirs) {
			if (path.startsWith(namedPath)) {
				...
				path = path.pathSplitter().array.remove(namedPath.pathSplitter().array.length).buildNormalizedPath();
			}
		}
		return path;
	}
...
public string[string] getPackageMap(string packageDir, Config config, string packageName = "", string destDir = "") {
	string[string] map;
	...
	foreach (string entry; packageDir.dirEntries(SpanMode.depth)) { //for each file node in the package dir
		foreach (string anchor, dest; config.anchorMap) { //for each anchor
	...
	foreach (string source, dest; map) {
		foreach (string namedDir; config.nameDirs) { //for each named directory
	...
	if (map.empty && !config.fallback.empty) {
		map[contentRoot] = config.fallback;
	}

	foreach (string source, dest; map) {
		if(source.length<contentRoot.length){
			// if the source path is shorter than,
			// and therefore a parent directory of,
			// the content root, we don't need it.
			// map.remove(source);
			continue;
		}
		if (source.length==contentRoot.length) { 
